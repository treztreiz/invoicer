...
* **Invariants:**
    ...
    * Invoice has **either** a recurrence **or** an installment plan (XOR). Not both; not neither if extension chosen.
...
- **API Platform resources:** expose `/customers`, `/documents`, `/installment-plans/*`, `/recurrences`, `/me`, `/reports/revenue.csv`, `/pdf/{id}`.
...
- **Validation:**
  ...
  - Recurrence vs installments: enforce **XOR** with DB constraint + app validation.
...
Review this RHF+Zod schema for the Document form. Ensure XOR rule and numeric validations. Minimal changes only.
<schema/snippet>
...
```sql
ALTER TABLE invoice
  ADD CONSTRAINT chk_invoice_xor
  CHECK ((recurrence_id IS NULL) <> (installment_plan_id IS NULL));
````
